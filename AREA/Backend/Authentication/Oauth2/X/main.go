...
func main() {
	err := loadXCredentials("credentials.json")
	if err != nil {
	...
	fmt.Println("Server is running at http://localhost:8085")
	if err := r.Run(":8085"); err != nil {
		log.Fatalf("Failed to run server: %v", err)
	}
}

func loadXCredentials(filename string) error {
	...
	XOauthConfig.ClientID = creds.Web.ClientID
	XOauthConfig.ClientSecret = creds.Web.ClientSecret
	XOauthConfig.RedirectURL = "http://localhost:8080/callback"
...
func handleLogin(c *gin.Context) {
	url := fmt.Sprintf("https://twitter.com/i/oauth2/authorize?response_type=code&client_id=%s&redirect_uri=%s&state=%s&scope=tweet.read",
		XOauthConfig.ClientID, XOauthConfig.RedirectURL, oauthStateString)
...
func handleFacebookCallback(c *gin.Context) {
	...
	payload := fmt.Sprintf("client_id=%s&redirect_uri=%s&client_secret=%s&code=%s",
		XOauthConfig.ClientID, XOauthConfig.RedirectURL, XOauthConfig.ClientSecret, code)
